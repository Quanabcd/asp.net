...
public partial class home_products_default_u_products_detail : System.Web.UI.UserControl
{
    ...
    public int icid = 0;
    public string alias = "";
    private DataTable prodetail = new DataTable();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string key_web = "";
        string t_web = "";
        dt = WEB.Items.getlisitem(WEB.Common.mod_product, lang, "valias='" + alias + "'", "");
        prodetail = dt;
        string image = "";
        if (dt.Rows.Count > 0)
        {
            ...
            ltkey.Text = vkey = dt.Rows[0]["vkey"].ToString();
            ...
            ltimg.Text += "<img class='product_featured_image' src='" + WebURL + "uploads/products/" + dt.Rows[0]["vimg"].ToString() + "' alt='" + dt.Rows[0]["vtitle"].ToString() + "'/>";
          
            if (dt.Rows[0]["vdesc"].ToString().Length>0)
            {
            ...
            ltlichtrinh.Text = dt.Rows[0]["vparam5"].ToString();
            ...
            {
                txtxuatxu.Text = "Đang được cập nhật ...";
            }
            ...
            if (Convert.ToInt32(dt.Rows[0]["iparam8"].ToString()) > 1)
            {
                ...
                {
                    //lthsx.Text = "<li class='status'><i class=\"fa fa-chevron-down\"></i>" + getcaption("lt_prdhsx") + ":&nbsp" + dthsx.Rows[0]["vname"].ToString() + "</li>";
                }
            }
    ...
    protected void loaditemsincate(string icid)
    {
        ...
        //string condition = " vcode ='" + TN.Common.mod_product + "' and istatus = 1";
    ...
    public void loadimg(int iid, string img)
    {
        ...
        if (dtimg.Rows.Count == 0)
        {
            ...
            str += "  <a href='javascript:void(0)' data-image='" + WebURL + "uploads/products/" + img + "' data-zoom-image='" + WebURL + "uploads/products/" + img + "'>";
            str += "  <img src='" + WebURL + "uploads/products/" + img + "' data-image='" + WebURL + "uploads/products/" + img + "' />";
            str += "  </a>";
            str += "  </div>";
        }
        else if (dtimg.Rows.Count > 0)
        {
            ...
            {
                str += "  <div class='item_img'>";
                str += "  <a href='javascript:void(0)' data-image='" + WebURL + "uploads/products/relative/" + dtimg.Rows[i]["vtitle"].ToString() + "' data-zoom-image='" + WebURL + "uploads/products/relative/" + dtimg.Rows[i]["vtitle"].ToString() + "'>";
                str += "  <img src='" + WebURL + "uploads/products/relative/" + dtimg.Rows[i]["vtitle"].ToString() + "' data-image='" + WebURL + "uploads/products/relative/" + dtimg.Rows[i]["vtitle"].ToString() + "' />";
                str += "  </a>";
                str += "  </div>";
            }
        }
    ...
    public string getprice(string fold, string fnew)
    {
        ...
        if (fnew != "0")
        {
            str += "<ins class='m-b-5'>" + TN.Common.FomatPrice(fnew, lang) + "</ins>";
        }
        else
            str += "<ins class='m-b-5'>Liên hệ</ins>";
        if (fold != fnew && fold != "0")
            str += "<div class='product_compare_at_price m-b-3'><span>Giá gốc: </span><del>" + TN.Common.FomatPrice(fold, lang) + "</del></div>";
        return str;
    }
    ...
    protected void loaditemrelative(int num)
    {
        ...
        string key = "";
        DataTable dt = new DataTable();
        dt = TN.Items.GetListItemInCateByPage(icid.ToString(), fieldsearch, key, TN.Common.mod_product, lang, "", "1", 0, num, " dupdate DESC ");
