...
var qq = function (element) {
    ...
    return {
        ...
        */
        ...
        setAttribute: function (text, value) {
...
qq.supportedFeatures = (function () {
    ...
    if (supportsDeleteFileCors == false) {
        if (qq.ie())
            supportsDeleteFileCors = true;
    }
...
qq.FineUploaderBasic = function (o) {
    this._options = {
        ...
        messages: {
            typeError: "File: \"{file}\" " + bds_lang.UploadImageControl.TypeError + ": {extensions}.",
            sizeError: "File: \"{file}\" " + bds_lang.UploadImageControl.SizeError + ": {sizeLimit}.",
            minSizeError: "File: \"{file}\" " + bds_lang.UploadImageControl.MinSizeError + ": {minSizeLimit}.",
            emptyError: "{file} is empty, please select files again without it.",
            noFilesError: "No files to upload.",
            tooManyItemsError: bds_lang.UploadImageControl.TooManyItemsError1 + " ({netItems}).  " + bds_lang.UploadImageControl.TooManyItemsError2 + ": {itemLimit}",//"Bạn đã upload quá nhiều ảnh cho phép ({netItems}).  Số lượng tối đa cho phép: {itemLimit}.",
...
qq.FineUploaderBasic.prototype = {
    ...
    _onSubmit: function (id, name) {
        ...
        if (this._options.validation.maxCount <= $(this._listElementView).children().length + 1) {
            $(this._find(this._element, 'button')).hide();
        }
    ...
    _onDeleteComplete: function (id, xhr, isError) {
        ...
        if (isError) {
            ...
            if (this._options.validation.maxCount >= $(this._listElementView).children().length - 1) {
                $(this._find(this._element, 'button')).show();
            }
        }
    },
    ...
    _onValidateBatchCallbackSuccess: function (validationDescriptors, items, params, endpoint) {
        var errorMessage,
            itemLimit = this._options.validation.itemLimit, trueCount = this._options.validation.trueCount,
            proposedNetFilesUploadedOrQueued = this._netUploadedOrQueued + validationDescriptors.length;
        
        if (itemLimit === 0 || proposedNetFilesUploadedOrQueued <= trueCount) {
            if (items.length > 0) {
                this._handleCheckedCallback({
                    name: "onValidate",
            ...
            errorMessage = this._options.messages.tooManyItemsError
                .replace(/\{netItems\}/g, proposedNetFilesUploadedOrQueued + (itemLimit - trueCount))
...
qq.FineUploader = function (o) {
    ...
    qq.extend(this._options, {
        ...
        text: {
            ...
            waitingForResponse: bds_lang.UploadImageControl.WaitingForResponse
        },
        ...
        deleteFile: {
            forceConfirm: false,
            confirmMessage: bds_lang.UploadImageControl.ConfirmMessage + " {filename}?",
            deletingStatusText: bds_lang.UploadImageControl.DeletingStatusText,
            deletingFailedText: bds_lang.UploadImageControl.DeletingFailedText
    ...
    if (!qq.supportedFeatures.uploading || (this._options.cors.expected && !qq.supportedFeatures.uploadCors)) {
        ...
        this._options.fileTemplateView = this._options.fileTemplateView.replace(/\{cancelButtonText\}/g, this._options.text.cancelButton);
        this._options.fileTemplateView = this._options.fileTemplateView.replace(/\{retryButtonText\}/g, this._options.text.retryButton);
        this._options.fileTemplateView = this._options.fileTemplateView.replace(/\{deleteButtonText\}/g, this._options.text.deleteButton);
        this._options.fileTemplateView = this._options.fileTemplateView.replace(/\{statusText\}/g, "");

        this._element = this._options.element;
        this._element.innerHTML = this._options.template;
        this._listElement = this._options.listElement || this._find(this._element, 'list');
        this._listElementView = this._options.listElementView || this._find(this._element, 'listView');

        $('.oldfilethumbnails').children().appendTo(this._listElementView);
        ...
        if (this._options.validation.maxCount <= $(this._listElementView).length) {
            $(this._find(this._element, 'button')).hide();
        }
    }
};
...
qq.extend(qq.FineUploader.prototype, {
    ...
    getItemViewByFileId: function (id) {
        var item = this._listElementView.firstChild;
    ...
    _removeFileItem: function (fileId) {
        ...
        var itemView = this.getItemViewByFileId(fileId);
        qq(item).remove();
        qq(itemView).remove();
    },
    ...
    **/
    ...
    _onComplete: function (id, name, result, xhr) {
        ...
        if (result.success) {
            ...
            this._addToListView(id, name, result.data, result.newUuid);
    ...
    _addToListView: function (id, name, image, uuid) {
        var item = qq.toElement(this._options.fileTemplateView);
        item.qqFileId = id;
        var fileElement = this._find(item, 'file');
        qq(fileElement).setAttribute("style", "background-image:url(" + image + ");");
        qq(fileElement).setAttribute("rel", this._options.svId + 'temp.' + uuid);
        qq(fileElement).setAttribute("status", 'new');

        if (this._options.display.prependFiles) {
            this._prependItem(item);
        }
        else {
            this._listElementView.appendChild(item);
        }
        this._filesInBatchAddedToUi += 1;
    },
    ...
    _bindCancelAndRetryEvents: function () {
        ...
        this._disposeSupport.attach(list, 'click', function (e) {
            ...
            if (qq(target).hasClass(self._classes.cancel) || qq(target).hasClass(self._classes.retry) || qq(target).hasClass(self._classes.deleteButton)) {
                ...
                if (item.qqFileId != undefined)
                    uuid = item.qqFileId;
                //else if ($(item).attr('rel') != undefined) {
                //    uuid = $(item).attr('rel');
                //    uuid = uuid.replace('.', '');
                //}
...
qq.DeleteFileAjaxRequestor = function (o) {
    ...
    return {
        sendDelete: function (id, uuid) {
            ...
            options.onDeleteComplete(id, null, false);

            return;
            /*
            if (qq.ie() && qq.firefox() == false && qq.chrome() == false) {
                var endpoint = options.endpointStore.getEndpoint(id) + '/' + uuid;
                options.onDelete(id);
                ...
                $.ajax({
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        options.onDeleteComplete(id, jqXHR, false);
                    },
                    success: function (data, stt, xhr) {
                        options.onDeleteComplete(id, xhr, !qq.indexOf(options.successfulResponseCodes, st) >= 0);
                    }
                });
            }
            else {
...
qq.UploadHandlerForm = function (o, uploadCompleteCallback, onUuidChanged, logCallback) {
    ...
    function registerPostMessageCallback(iframe, callback) {
        ...
        corsMessageReceiver.receiveMessage(iframeName, function (message) {
            ...
            if (uuid && onloadCallbacks[uuid]) {
                ...
                console.log("'" + message + "' does not contain a UUID - ignoring.");
                return false;
            }
        });
    }
    ...
    function parseResponse(id, innerHtmlOrMessage) {
        ...
        try {
            ...
            if (response.newUuid !== undefined) {
                console.log("Server requested UUID change from '" + uuids[id] + "' to '" + response.newUuid + "'");
    ...
    */
    function createIframe(id) {
        ...
        var iframe = qq.toElement('<iframe src="about:blank" name="' + iframeName + '" style="width:164px; height:170px;" />');
    ...
    */
    function createForm(id, iframe) {
        ...
        var protocol = options.demoMode ? "GET" : "POST";
        var form = qq.toElement('<form method="' + protocol + '" enctype="multipart/form-data" target="' + iframe.name + '"></form>');
        var endpoint = options.endpointStore.getEndpoint(id);
        var url = endpoint;
        ...
        //form.setAttribute('action', iframe.src);
        form.style.display = 'none';

        var inputfromiframe = qq.toElement('<input type="text" name="fromiframe" value="true"/>');
        form.appendChild(inputfromiframe);
